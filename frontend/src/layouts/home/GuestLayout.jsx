...
import { FaPhone, FaTooth } from "react-icons/fa";
...
export default function GuestLayout() {
    ...
    <>
        {(user.role === 'admin' || user.role === 'doctor') ? (
            <>
                <Link to='/admin/dashboard' className='border-blue-400 bg-transparent text-blue-400 border p-2 rounded-md'>
                    Dashboard
                </Link>
                <button onClick={logout} className='border-blue-400 bg-transparent text-blue-400 border p-2 rounded-md'>
                    Logout
                </button>
            </>
        ) : user.role === 'patient' ? (
            <>
                <><span className='text-2xl text-blue-400'>{user.first_name} {user.last_name}</span>
                <button onClick={logout} className='border-blue-400 bg-transparent text-blue-400 border p-2 rounded-md'>
                    Logout
                </button></>
            </>
        ) : <Link to='/login' className='border-blue-400 bg-transparent text-blue-400 border p-2 rounded-md'>
        Login
    </Link>}
    </>
) : (
    <Link to='/login' className='border-blue-400 bg-transparent text-blue-400 border p-2 rounded-md'>
        Login
            ...
            <div className='grad-background p-10 text-white flex items-center justify-between'>
                ...
                <div>
                    <h3 className='text-3xl mb-4'>Subscribe</h3>
                    <div className='flex'>
