...
export default function UpdateMedicine() {
  ...
  const fetchMedicine = async () => {
    try {
        const response = await axiosClient.get(`${import.meta.env.VITE_BACKEND_URL}api/medicines/${id}`);
        setMedicine(response.data.medicine);
         setFormData({
          name: response.data.medicine.name,
          category: response.data.medicine.category,
          quantity: response.data.medicine.quantity,
          selling_price:response.data.medicine.selling_price,
          buying_price:response.data.medicine.buying_price,
          status: response.data.medicine.status,
      })
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
        const response = await axiosClient.post(`${import.meta.env.VITE_BACKEND_URL}api/medicines/${id}`,{...formData,_method:'PUT'});
...
const handleChange = (e)=>{
    ...
    return (
        <div className='p-4 bg-slate-800 min-h-screen text-white'>
            ...
            {!medicine ? <Loading/> :<form onSubmit={handleSubmit} className='bg-slate-700 p-5 rounded-lg mt-5'>
            <div className='grid grid-cols-2'>
                    <div className='w-full lg:w-1/2 px-3 mb-6'>
                        <label htmlFor="name" className='block'>Name:<span className='text-red-500'>*</span></label>
                        <input type="text" id="name" name="name" value={formData.name} onChange={handleChange} required className='bg-slate-800 p-2 border border-slate-600 rounded-md text-white outline-none' placeholder='Medicine Name' />
                ...
                <button type="submit" className='bg-blue-500 hover:bg-blue-700 text-white  py-2 px-4 rounded mt-3'>{loading ? <AiOutlineLoading3Quarters className='text-white animate-spin' /> : 'Update'}</button>
